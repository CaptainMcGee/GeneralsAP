...
function Resolve-ReferenceRuntimeDirectory {
    ...
    if ((Test-Path -LiteralPath (Join-Path $candidate "generalszh.exe")) -or
        (Test-Path -LiteralPath (Join-Path $candidate "INIZH.big"))) {
        return $candidate
    }
...
function Sync-ReferenceRuntimeAssets {
    ...
    foreach ($entry in $requiredAssetEntries) {
        ...
        if (-not (Test-Path -LiteralPath $entryPath)) {
            $missingAssets.Add($entry)
        }
    }
    if ($missingAssets.Count -gt 0) {
        $message = ($missingAssets | ForEach-Object { " - $_" }) -join [Environment]::NewLine
        throw "Reference runtime directory is missing required game assets at '$sourceFull':`n$message"
    }
...
# Regenerate Archipelago.ini with granularity overrides when any granularity is non-default.
# This replaces the runtime profile's pre-baked INI with a freshly generated one that
# decomposes categorized groups into individual or per-general items as requested.
# When all granularities are "categorized" (default), the profile's INI is used as-is.
$anyGranularityOverride = ($UnitGranularity -ne "categorized") -or ($BuildingGranularity -ne "categorized") -or ($UpgradeGranularity -ne "categorized")
if ($anyGranularityOverride) {
    $generateIniScript = Join-Path $repoRoot "scripts\archipelago_generate_ini.py"
    $runtimeArchipelagoIni = Join-Path $runtimeDir "Data\INI\Archipelago.ini"
    $pythonCommand = Resolve-PythonCommand
    $pythonExe = $pythonCommand[0]
    $pythonArgs = @()
    if ($pythonCommand.Length -gt 1) {
    ...
    $pythonArgs += @(
        ...
        "--output", $runtimeArchipelagoIni,
        "--unit-granularity", $UnitGranularity,
        "--building-granularity", $BuildingGranularity,
        "--upgrade-granularity", $UpgradeGranularity
    )
    Invoke-External -FilePath $pythonExe -Arguments $pythonArgs -WorkingDirectory $repoRoot
    Write-Host ("Regenerated Archipelago.ini with granularity: units={0}, buildings={1}, upgrades={2}" -f $UnitGranularity, $BuildingGranularity, $UpgradeGranularity)
}
...
if ($anyGranularityOverride) {
    Write-Host ("Archipelago.ini: regenerated with granularity (units={0}, buildings={1}, upgrades={2})" -f $UnitGranularity, $BuildingGranularity, $UpgradeGranularity)
}
else {
