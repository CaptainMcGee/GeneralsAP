...
param(
    ...
    [string]$Fixture = "",
    [int]$RandomUnlockCount = 10,
    [int]$RandomUnlockSeed = 0,
    [string]$StarterGeneral = "",
    [int]$StartingCashBonus = 100000,
    [double]$ProductionMultiplier = 3.0,
    [switch]$NoZoomLimit,
    [switch]$AiStress,
...
if (-not $NoBridge) {
    ...
    # When granularity overrides are active, tell the bridge to read group names
    # from the regenerated runtime INI so synthetic received-items use valid names.
    ...
    if ($anyGranularityOverride) {
        $runtimeIniPath = Join-Path $runtimeDir "Data\INI\Archipelago.ini"
        if (Test-Path -LiteralPath $runtimeIniPath -PathType Leaf) {
            $bridgeArguments += @("--ini-path", $runtimeIniPath)
        }
    }
