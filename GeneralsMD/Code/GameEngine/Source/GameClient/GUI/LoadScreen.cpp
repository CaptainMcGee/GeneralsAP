...
//	m_overlayReticleCircleLineInner->winHide(TRUE);
	...
	if(TheGameLODManager && TheGameLODManager->didMemPass())
	{
		...
		if ( TheGlobalData && TheGlobalData->m_skipCutscenesForDebug )
			m_videoStream->frameGoto( m_videoStream->frameCount() ); // skip to end
#endif
...
#if defined(RTS_DEBUG)
		Int delay = ( TheGlobalData && TheGlobalData->m_skipCutscenesForDebug ) ? 0 : mission->m_voiceLength * 1000;
#else
		Int delay = mission->m_voiceLength * 1000;
#endif
		Int begin = timeGetTime();
