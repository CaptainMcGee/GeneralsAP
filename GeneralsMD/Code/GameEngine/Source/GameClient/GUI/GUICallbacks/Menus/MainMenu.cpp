...
static NameKeyType buttonArchipelagoID = NAMEKEY_INVALID;
...
static GameWindow *buttonArchipelago = nullptr;
...
static Bool apShellReviewAutoOpenHandled = FALSE;
static Bool apShellReviewCaptureHandled = FALSE;
...
static AsciiString readAPShellReviewAutoOpenTarget()
{
	std::ifstream input("UserData\\Archipelago\\APShellReviewOpen.txt");
	if (!input)
	...
	return AsciiString(line.substr(begin, end - begin).c_str());
}

static const char *resolveAPShellReviewMenu(const AsciiString &target)
{
	if (target.compareNoCase("hub") == 0)
		return "Menus/ArchipelagoHub.wnd";
	if (target.compareNoCase("connect") == 0)
		return "Menus/APConnect.wnd";
	if (target.compareNoCase("mission-intel") == 0)
		return "Menus/APMissionIntel.wnd";
	if (target.compareNoCase("check-tracker") == 0)
		return "Menus/APCheckTracker.wnd";
	return nullptr;
}

static void maybeAutoOpenAPShellReviewTarget()
{
	if (apShellReviewAutoOpenHandled || buttonPushed || isShuttingDown || startGame)
		return;

	const AsciiString target = readAPShellReviewAutoOpenTarget();
	if (target.isEmpty())
		return;

	apShellReviewAutoOpenHandled = TRUE;
	const char *wndName = resolveAPShellReviewMenu(target);
	if (wndName == nullptr)
		return;

	DEBUG_LOG(("AP shell review auto-open: %s", target.str()));
	buttonPushed = TRUE;
	dontAllowTransitions = TRUE;
	TheShell->push(wndName);
}

static void maybeCaptureAPShellReviewTarget()
{
	if (apShellReviewCaptureHandled || buttonPushed || isShuttingDown || startGame)
		return;

	const AsciiString target = readAPShellReviewAutoOpenTarget();
	if (target.isEmpty() || target.compareNoCase("mainmenu") != 0)
		return;

	apShellReviewCaptureHandled = TRUE;
	DEBUG_LOG(("AP shell review capture: %s", target.str()));
...
void MainMenuInit( WindowLayout *layout, void *userData )
{
	...
	isShuttingDown = FALSE;
	startGame = FALSE;
	apShellReviewAutoOpenHandled = FALSE;
	apShellReviewCaptureHandled = FALSE;
...
//	buttonTRAININGID = TheNameKeyGenerator->nameToKey( "MainMenu.wnd:ButtonTRAINING" );
	...
	buttonArchipelagoID = TheNameKeyGenerator->nameToKey( "MainMenu.wnd:ButtonArchipelago" );
	...
	buttonArchipelago = TheWindowManager->winGetWindowFromId( parentMainMenu, buttonArchipelagoID );
...
//	buttonTRAINING = TheWindowManager->winGetWindowFromId( parentMainMenu, buttonTRAININGID );
	...
	if( buttonArchipelago )
		GadgetButtonSetAltSound(buttonArchipelago, "GUIGenShortcutClick");
...
//	}
	...
	if (!justEntered && TheShell->isAnimFinished() && TheTransitionHandler->isFinished())
	{
		maybeAutoOpenAPShellReviewTarget();
		maybeCaptureAPShellReviewTarget();
	}
...
*/			else if(controlID == skirmishID)
		...
		case GBM_SELECTED:
		{
			...
			else if( controlID == buttonArchipelagoID )
			{
				if(dontAllowTransitions)
					break;
				dontAllowTransitions = TRUE;
				buttonPushed = TRUE;
				TheShell->push( "Menus/ArchipelagoHub.wnd" );
			}
