...
void Object::setDisabledUntil( DisabledType type, UnsignedInt frame )
{
	...
	// Spawned cluster units are unconditionally immune to all disable effects
	// (EMP, hacker, leaflet drop, etc.) regardless of protection registry state.
	if ( TheUnlockableCheckSpawner != nullptr
		&& TheUnlockableCheckSpawner->isSpawnedUnit( this ) )
	{
		return;
