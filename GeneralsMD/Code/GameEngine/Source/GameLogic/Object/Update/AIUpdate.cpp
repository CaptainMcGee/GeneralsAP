...
Bool AIUpdateInterface::isAllowedToRespondToAiCommands(const AICommandParms* parms) const
{
  ...
  // AUTHOR, LORENZEN... 5/15/03
	...
	// UnlockableCheckSpawner: spawned units only accept commands from the spawner itself.
	// CMD_FROM_SCRIPT is used by both the spawner and map triggers, so we also check that the
	// spawner is actively issuing a command (m_spawnerCommandInProgress) to block map scripts.
	if (TheUnlockableCheckSpawner && TheUnlockableCheckSpawner->isSpawnedUnit(getObject()))
	{
		if (parms->m_cmdSource != CMD_FROM_SCRIPT || !TheUnlockableCheckSpawner->isSpawnerCommandInProgress())
			return FALSE;
	}
...
void AIUpdateInterface::privateMoveAwayFromUnit( Object *unit, CommandSourceType cmdSource )
{
	...
	// Spawned units must hold their guard positions and never dodge other units.
	if (TheUnlockableCheckSpawner && TheUnlockableCheckSpawner->isSpawnedUnit(getObject()))
	{
		return;
	}
...
Object* AIUpdateInterface::getNextMoodTarget( Bool calledByAI, Bool calledDuringIdle )
{
	...
	// UnlockableCheckSpawner: block target acquisition only during forced retreat (exceeded MaxChaseRadius).
	// The engine's AIGuardMachine handles all radius-based targeting decisions during normal guard behavior.
	if (TheUnlockableCheckSpawner && TheUnlockableCheckSpawner->isSpawnedUnit(obj))
	{
		if (TheUnlockableCheckSpawner->isSpawnedUnitInForcedRetreat(obj))
			return nullptr;
	}
