...
)
{
	...
	{
		static Bool s_warnedLargeSmallGeometry = FALSE;
		if (!s_warnedLargeSmallGeometry)
		{
			DEBUG_LOG(("object is too large to use a 'small' geometry, truncating size to cellsize"));
			s_warnedLargeSmallGeometry = TRUE;
		}
		radius = halfCellSize;
	}
...
void PartitionContactList::processContactList()
{
	for (PartitionContactListNode* cd = m_contactList; cd; cd = cd->m_next)
	{
		...
		// Spawned cluster units: skip collision with other spawned units and
		// enemy general units, but collide normally with player units.
		if (TheUnlockableCheckSpawner != NULL)
		{
			Bool objIsSpawned = TheUnlockableCheckSpawner->isSpawnedUnit(obj);
			Bool otherIsSpawned = TheUnlockableCheckSpawner->isSpawnedUnit(other);
			if (objIsSpawned || otherIsSpawned)
			{
				if (objIsSpawned && otherIsSpawned)
					continue;
				const Object* nonSpawned = objIsSpawned ? other : obj;
				Player* localPlayer = ThePlayerList ? ThePlayerList->getLocalPlayer() : NULL;
				if (localPlayer == NULL || nonSpawned->getControllingPlayer() != localPlayer)
					continue;
			}
		}
