...
StateReturnType AIAttackFireWeaponState::onEnter()
{
	...
	if ((adjust & MAA_Action_Ok) == 0)
	{
		...
		{
			DEBUG_LOG( ( "[Archipelago][BLOCKED] AttackFireWeaponState unit %d: mood matrix rejected fire (adjustment=0x%x)",
				(Int)obj->getID(), adjust ) );
		}
		return STATE_FAILURE;
	}
	Object *victim = getMachineGoalObject();
...
StateReturnType AIAttackState::onEnter()
{
	...
	{
		if (TheUnlockableCheckSpawner && TheUnlockableCheckSpawner->isSpawnedUnit(source))
		{
			DEBUG_LOG( ( "[Archipelago][BLOCKED] AIAttackState::onEnter unit %d: mood matrix rejected attack (adjustment=0x%x)",
				(Int)source->getID(), ai->getMoodMatrixActionAdjustment(MM_Action_Attack) ) );
		}
		return STATE_SUCCESS;
	}
