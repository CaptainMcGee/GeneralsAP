...
- Latest repeated ordered non-human release gate passed on May 4, 2026 UTC at `2026-05-04T13:15:17Z`: 14 passed / 0 failed without the fast real-AP shortcut and with `-RunIntegratedRealApRuntimeSmoke` plus `-RunSpawnedMaterializationSmoke`, including PR scope audit, generated-output cleanliness, bridge session mismatch rejection, incremental `ReceivedItems`, full real local AP server smoke, package root/zip validation, legal-runtime guarded mission/cluster completion, selected spawned check materialization, live AP network seeding/submission into the clean runtime, fresh AP reconnect persistence, and AP numeric ID translation.
