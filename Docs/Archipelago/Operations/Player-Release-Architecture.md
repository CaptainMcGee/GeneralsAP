...
- `scripts/run_generalsap_demo_director.ps1` creates the current human-like non-human demo proof. It uses `human_demo_tank`, proves selected Tank challenge cluster materialization, proves guarded mission/cluster completion to AP numeric IDs, optionally proves live local AP network submission, and writes `build\archipelago\demo\Demo-Proof.json`. Interpret that artifact through `Docs\Archipelago\Operations\Demo-Proof-Review.md`.
