...
- `scripts/archipelago_bridge_executable_smoke.py` verifies the bridge executable can materialize supplied slot data, write inbound metadata, reject unknown runtime keys, reject seed/session mismatch reuse without `--reset-session`, merge duplicate completions idempotently, and translate selected future-family fixture checks (`capture.tank.b001`, `supply.tank.p02.t02`) without enabling production capture/supply gameplay.
- `scripts/archipelago_bridge_network_smoke.py` verifies the same bridge executable can speak the AP 0.6.7 websocket seam against a fake AP server, receive `slot_data` and incremental `ReceivedItems`, write the same file contract, submit `LocationChecks`, avoid duplicate submissions across reconnects, and keep Boss victory as `StatusUpdate` only even if the fake server advertises the Boss marker as a known ID.
...
- The package uses an allowlist, validates manifest schema, scans both root and zip output for forbidden retail archive types, rejects unsafe zip paths, rejects unclaimed `payload/Game` files, and filters/rejects transient APWorld artifacts such as `__pycache__/`, `.pyc`, `.log`, `.tmp`, and `host.yaml`.
...
Current package tooling ships the APWorld source folder as `payload/APWorld/generalszh/` and records `apworldPayload = "folder"` in the release manifest. It does not currently build a zipped `generalszh.apworld` inside the overlay package; a separate host/generator artifact is still expected before public alpha distribution.
...
- Latest repeated ordered non-human release gate passed on May 4, 2026 UTC at `2026-05-04T13:57:36Z`: 14 passed / 0 failed without the fast real-AP shortcut and with `-RunIntegratedRealApRuntimeSmoke` plus `-RunSpawnedMaterializationSmoke`, including PR scope audit, stronger generated-output cleanliness, bridge session mismatch rejection, incremental `ReceivedItems`, real local AP server smoke with cache locking/port retry, package root/zip validation with sibling-entry rejection, packaged-bridge package smoke, legal-runtime guarded mission/cluster completion with completion-proof process liveness, selected spawned check materialization, live AP network seeding/submission into the clean runtime, fresh AP reconnect persistence, and AP numeric ID translation.
