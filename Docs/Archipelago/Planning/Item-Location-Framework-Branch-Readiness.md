...
- Latest strongest local run `2026-05-05T01:34:58Z`: 16 passed / 0 failed without `-FastRealApSmoke` and with `-RunIntegratedRealApRuntimeSmoke` plus `-RunSpawnedMaterializationSmoke` against the local Steam/TUC Zero Hour runtime at `C:\Program Files (x86)\Steam\steamapps\common\Command & Conquer Generals - Zero Hour`. The ordered gate now includes PR scope audit, packaged bridge build, AP data/world suite, Logic Foundry export handoff smoke, generated-output cleanliness, packaged-bridge file smoke, fake AP network smoke, real local AP server smoke with shared-cache locking and port retry, full AP world simulated completion smoke, package root/zip validation with sibling-entry rejection, clean-runtime fixture smoke, prepared game runtime build, clean-runtime completion-proof process liveness guard, legal-runtime auto-completion smoke, selected spawned check materialization, live AP network seeding/submission into the clean runtime, future-family fixture bridge checks, APWorld payload dependency checks, and bundled `staging_stub` package rejection.
- Latest local run `2026-05-07T12:59:05Z`: 17 passed / 0 failed with `-BaseRuntimeDir`, `-RunIntegratedRealApRuntimeSmoke`, and `-RunSpawnedMaterializationSmoke` against the same Steam/TUC runtime. This run also verified the economy-effect contract after `Progressive Starting Money` and `Progressive Production` were moved to `useful` classification until mission gates consume them. A prior `windows_demo_run.ps1 -NoBridge -NoLaunch` attempt exposed a wrapper hang after successful prepare; the wrapper now invokes `windows_debug_prepare.ps1` directly with an argument array, and the legal-runtime no-launch prep completes cleanly.
...
- Latest targeted PR self-review checks passed: `test_archipelago_world_contract.py`, `test_archipelago_data_pipeline.py`, `archipelago_run_checks.py`, package fixture smoke, real local AP server smoke, Logic Foundry export handoff smoke, full local legal-runtime release gate, and `archipelago_pr_scope_audit.py`. The ordered non-human release runner calls `archipelago_pr_scope_audit.py` by default before build/package/runtime gates. `.github/workflows/validate-archipelago-data.yml` now defines Windows AP framework contract and `GeneralsMD` runtime compile gates; checkpoint `7237ad3` passed both push and PR GitHub runs, including AP data, AP framework contracts, Logic Foundry export handoff, smoke-artifact staging, and runtime compile.
